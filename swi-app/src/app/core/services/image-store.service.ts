...
import { SWIDBService } from "app/core";
...
export class ImageStoreService {
  ...
  public async add(swiId: string, image: string | SWIImage, compress: boolean = false): Promise<SWIImage> {
    ...
    const swiStoreImg = this.convertToStoreImage(swiId, swiImg);
    try {
  ...
  public async addAll(swi: SWIHeader, swiKey: string, compress: boolean = false): Promise<void> {
    if (swi.swiImages) {
      ...
      for (var i = 0; i < swi.swiImages.length; i++) {
        // Check to see if the image is already in the store
        const element = swi.swiImages[i];
        const isLoaded = await this.isLoaded(element.key);
        if (!isLoaded) {
          await this.add(swiKey, element, compress);
          console.log("New image added");
        }
        console.log(`Image ${i + 1} of ${swi.swiImages.length}`);
      }
    }
  }
  ...
  public async duplicateImage(imageKey: string, newSWIKey: string): Promise<string> {
    ...
    const origImg = await this.imageStore.get(imageKey);
    console.log("Orig Img", origImg);
    const newImg = Object.assign({}, origImg);
    newImg.key = new GUID().value;
    newImg.swiKey = newSWIKey;
    console.log("New Img", newImg);
    const result = await this.imageStore.add(newImg);
    console.log("New Img", newImg);
    return result;
  }
  ...
  private async isLoaded(imageKey: string): Promise<boolean> {
    const result = await this.imageStore.get(imageKey);
    return (result != null);
  }
  ...
  public getSWIImageKeys(swi: SWIHeader): string[] {
    ...
    for (const group in swi.stageGroups) {
      if (swi.stageGroups.hasOwnProperty(group)) {
        const element = swi.stageGroups[group];
  ...
  private convertToStoreImage(swiId: string, swiImage: SWIImage): SWIStoreImage {
    const storeImg: SWIStoreImage = new SWIStoreImage(swiImage.value);
  ...
  private async compressImages(swiImg: SWIImage): Promise<SWIImage> {
    ...
    const image = await this.imageService.generateImage(swiImg.value);
  ...
  private async generateThumbnail(swiImg: SWIImage): Promise<SWIImage> {
    ...
    const thumb = await this.imageService.generateThumbnail(swiImg.value);
